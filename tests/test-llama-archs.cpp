...
static int test_dsa_kpool(const size_t seed, const ggml_log_level log_level) {
    struct user_data_t {
        struct {
            ggml_log_callback callback;
            void * user_data;
...
//
static int test_mtp(const size_t seed, const ggml_log_level log_level) {
    struct user_data_t {
        struct {
            ggml_log_callback callback;
            void * user_data;
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        if (arch == LLM_ARCH_UNKNOWN || arch == LLM_ARCH_GLM5NEXT) {
            ret |= test_dsa_kpool(seed, log_level);
            ret |= test_mtp(seed, log_level);
        }
        ret |= test_backends(arch, seed, log_level);
        return ret;
