...
//   - WHICH of a set of tied columns is selected is likewise unconstrained. Some builds'
//     cmp_top_k breaks ties on the lower id and some do not, so a backend must not be
//     required to reproduce a particular tie SET either.
...
struct test_top_k : public test_case {
    ...
    void initialize_tensors(ggml_context * ctx) override {
        // Seeded from the case's own parameters, not from std::random_device. For the sparse
        // fills below, WHERE the selectable columns land is what decides whether a backend
        // that mishandles the unselectable ones is caught, so a random seed makes this a
        // probabilistic guard whose failures cannot be reproduced on rerun.
        uint32_t seed = 0x9e3779b9u;
        auto mix = [&seed](uint64_t v) {
            seed ^= (uint32_t) v + 0x9e3779b9u + (seed << 6) + (seed >> 2);
        };
        mix((uint64_t) ne[0]); mix((uint64_t) ne[1]); mix((uint64_t) ne[2]); mix((uint64_t) ne[3]);
        mix((uint64_t) k); mix((uint64_t) ties); mix((uint64_t) fill); mix((uint64_t) n_sel);
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // selection kernel, k > 32 the argsort fallback.
    //
    // The widths matter as much as k does:
    //   - ne0 below the SLM kernel's 128-wide workgroup leaves some work-items with no column
    //     at all, so every slot they contribute is a sentinel. ne0 = 64, k = 2, nrows = 64 is
    //     the shape the defect was found on (the glm5next DSA k-pool selector).
    //   - ne0 == k is the tight case for any "fill the holes with unused columns" repair: the
    //     filler columns and the selected columns are drawn from the same k values, so a hole
    //     is guaranteed to collide with a selection unless the repair really checks.
    //   - nrows == 1 is decode.
    // Backend selection does not depend on nrows here: next_power_of_2(ne0) <= 16384 holds for
    // every width below, which satisfies supports_op's few-rows clause on its own.
    for (int k : {1, 2, 4, 7, 16, 32, 33, 64, 100}) {
        for (int64_t cols : {2, 8, 16, 32, 64, 127, 128, 1024, 8192}) {
            if (k > cols) {
                continue;
            }
            for (int64_t nrows : {1, 4, 32, 64}) {
