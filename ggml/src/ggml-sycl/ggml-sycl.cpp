        ...
        cgh.parallel_for(
            ...
            [=](sycl::nd_item<1> item_ct1) {
                ...
                if (tid == 0) {
                    ...
                    for (int i = 0; i < k; i++) {
                        dst_idx_row[i] = final_idx[i] >= 0 ? final_idx[i] : (int32_t) (i < ncols ? i : 0);
                    }
...
static void ggml_sycl_top_k_check(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    // DEBUG INSTRUMENT (GGML_SYCL_SET_ROWS_CHECK=1): report the index range TOP_K produced.
    ...
    for (int64_t i = 0; i < cnt; i++) { lo = std::min(lo, v[i]); hi = std::max(hi, v[i]); }
...
static void ggml_backend_sycl_graph_compute_impl(ggml_backend_sycl_context * sycl_ctx, ggml_cgraph * cgraph) {
    ...
    // DEBUG INSTRUMENT (GGML_SYCL_SYNC_EACH_OP=1): synchronise after every dispatched node so a
    // faulting/non-terminating kernel is attributed to the node that issued it instead of to the
    // next hard sync point in the graph.
