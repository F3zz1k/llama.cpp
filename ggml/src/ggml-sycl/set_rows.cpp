...
void ggml_sycl_op_set_rows(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    // SET_ROWS writes dst row `idx`; an index outside [0, dst->ne[1]) is an out-of-bounds
    // device write, which surfaces as a GPU page fault attributed to some later op. Report the
    // range and abort here instead, where the producing node is still identifiable.
    {
