...
`server_tokens::has_media()` (new, in `server-common.h`) reports whether *this* prompt contains media chunks (`!map_idx_to_media.empty()`) — the per-request signal, vs the server-wide `has_mtmd`. As originally shipped ("Option A"), the auto hooks gated on `!has_media()`: an `--mmproj` server cached all its **text-only** turns and skipped any turn carrying an image, because identical token-IDs don't identify image content (image cells are `LLAMA_TOKEN_NULL` placeholders) and caching them blind would violate invariant 2.

**Superseded:** the `auto-disk-kvcache-mm` branch removes that gate and makes media prompts first-class — per-chunk identity records (a content hash of the raw uploaded bytes + the chunk's shape) are folded into the block chain and byte-verified at restore, and the manual `/slots` endpoints' server-wide `check_no_mtmd` 501 became a per-slot gate. See [`03-multimodal-cache.md`](03-multimodal-cache.md). Text-only prompts still hash, name, and serialize bit-identically to what this document describes.
...
- **All-miss workload cost.** On a stream of cache *misses*, the force-on-miss rescan does an O(#files) `directory_iterator` per request. Bounded by `--slot-save-max-count` when a cap is set (e.g. 64), but at an unbounded/very large cap + high QPS it's worth a smarter "nothing new since last scan" short-circuit or an inotify tier.
...
- **No automated tests** (as of this commit — validation was end-to-end: cold-process restore, cross-process visibility, attention regression, LRU). *Since addressed on `auto-disk-kvcache-mm`:* the hashing, file-format, and `server_tokens` helpers have unit tests (`tests/test-auto-hash.cpp`, `tests/test-slot-meta.cpp` incl. a fuzz mode, `tests/test-server-tokens.cpp`) and the server behaviour is covered by `tools/server/tests/unit/test_slot_save_auto.py`.
