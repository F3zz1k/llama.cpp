...
New flags `--slot-save-max-count` and `--slot-save-max-mb` (both default **0** = unlimited; negatives rejected at parse). Eviction is **opt-in and scoped to the `--slot-save-auto` cache** — a plain manual `--slot-save-path` save never reaches this store and never deletes anything (upstream behaviour preserved).

### How it works
When `--slot-save-auto` is set and a cap is configured, then after a save enumerate the slot-save dir, pair each state file with its `.logits` sidecar into one `slot_save_unit` (evicted together), sort by mtime, and evict oldest until under both caps. The just-written unit is never evicted. A **single** snapshot larger than the byte cap is rejected (the save returns an error) rather than cascade-evicting every other valid snapshot to make room for something that can't fit.
...
- **Dedicated-directory contract.** With `--slot-save-auto` + a cap set, `--slot-save-path` is treated as server-owned; this is documented in the flag help and the function header. Without `--slot-save-auto`, or with no caps (the default), nothing is ever deleted.
